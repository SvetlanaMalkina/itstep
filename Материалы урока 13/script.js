...
class NewsFeed {
    ...
    findTag(str){
        return this.arr.reduce((accum,item)=>{
            let rightBracken=item.indexOf('>')
            debugger
            let tag=item.substring(1,rightBracken)
...
let newsObj = new NewsFeed([
    '<p>Новости спорта</p>',
    '<h2>Погода на завтра</h2>',
    '<a data-date="25-06-2020">Происшествия за #сегодня</a>',
    '<div>Новости политики</div>'
])
console.log(newsObj.print());
newsObj.newPush('<span>Последние новости часа</span>');
console.log(newsObj);
//newsObj.newDelete(2);
console.log(newsObj.findTag('div'));
console.log(newsObj);
console.log(newsObj.getNewsCount);